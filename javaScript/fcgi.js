...
function showroadname(result)
{
	...
	s = s+"<h1 style=\"margin:0;padding:10px 0 9px 7px;font:bold 16px arial,sans-serif\"> 驾车前往 终点 的路线</h1>"
	s = s+"<div class=\"barDiv2\"></div>";
	s = s+"<div style=\"text-align: center;\">距离"+parseFloat(distance[0]).toFixed(2)+"公里， 时间约"+parseInt(time[0])+"分钟</div>";
	s = s+"<div class=\"barDiv2\"></div>";
	s = s+"<div><div style=\"cursor:pointer;background:url(http://api.map.baidu.com/img/trans_icons.png) no-repeat 5px 5px;padding:8px 0 8px 33px;line-height:15px\"><span style=\"white-space:nowrap\">起点</span></div></div>";
	s = s+"<div class=\"barDiv2\"></div>";
    for (var i = 1; i < strs.length; i ++){
    	var temp = strs[i].split(",");
    	if(temp[3] != pre && temp[3] != "noname") {
    		count = count+1;
        	s = s+"<li><a id=\""+count+"\" onclick=\"select(this)\">"+(count) + ". 进入" + temp[3]+"</a></li>";
        	s = s+"<div class=\"barDiv2\"></div>";
        	pre = temp[3];
        }
    }
    s = s+"<div style=\"cursor:pointer;background:url(http://api.map.baidu.com/img/trans_icons.png) no-repeat 5px -21px;padding:8px 0 8px 33px;line-height:15px\"><span style=\"white-space:nowrap\">终点</span></div>";
    s = s+"<div class=\"barDiv2\"></div>";
